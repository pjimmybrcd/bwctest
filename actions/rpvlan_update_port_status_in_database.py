...
class RpvlanUpdateMacAuthFailureDatabaseAction(actions.SessionAction):
  ...
  def run(self, ip, ap_name, port, action):

     if action=='add':
         self.process_add_port(ip, ap_name, port)

     if action=='remove':
         self.process_remove_port(ip, ap_name, port)

     # TODO: Report errors like database failure!
     return (True)

  def process_add_port(self, ip, ap_name, port):
  ...
  def process_remove_port(self, ip, ap_name, port):
        connection = pymysql.connect(
             host=self._db_addr, 
             user=self._db_user,      
             passwd=self._db_pass,  
